...
# Here you can find all the labels of LSVRC12
...
BASIC_TRANSFORM = T.Compose([T.Resize(DEFAULT_BASE_SIZE), T.CenterCrop(DEFAULT_BASE_SIZE),T.ToTensor()])
CENTER_CROP_TRANSFORM = T.Compose([T.Resize(DEFAULT_BASE_SIZE), T.CenterCrop(DEFAULT_CROP_SIZE), T.ToTensor()])
RAND_CROP_TRANSFORM = T.Compose([T.Resize(DEFAULT_BASE_SIZE), T.RandomCrop(DEFAULT_CROP_SIZE), T.ToTensor()])

def get_base_transform(image_size: int = DEFAULT_BASE_SIZE):
    return T.Compose([T.Resize(image_size), T.CenterCrop(image_size), T.ToTensor()])


# preprocess = T.Compose([
#     T.Resize(DEFAULT_BASE_SIZE),
#     T.CenterCrop(DEFAULT_CROP_SIZE),
#     T.ToTensor(),
#     T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
# ])

# class CropsTransform:
#     """Crop image in a spesific place"""
#     def __init__(self, top_start: int,left_start: int,height: int = DEFAULT_CROP_SIZE,width: int = DEFAULT_CROP_SIZE):
...
#         for i in range (self.top_start, DEFAULT_BASE_SIZE - self.height):
#             for j in range(self.top_start, DEFAULT_BASE_SIZE - self.height):
#         self.top += 1 % self.height
#         self.left += 1 % self.height
#         return F.crop(img, self.top,self.left,self.height,self.width)


IMAGENET_LABELS = {"0": ["n01440764", "tench"], "1": ["n01443537", "goldfish"], "2": ["n01484850", "great_white_shark"],
                   ...
                   "69": ["n01768244", "trilobite"], "70": ["n01770081", "harvestman"], "71": ["n01770393", "scorpion"],
                   ...
                   "83": ["n01798484", "prairie_chicken"], "84": ["n01806143", "peacock"], "85": ["n01806567", "quail"],
                   "86": ["n01807496", "partridge"], "87": ["n01817953", "African_grey"], "88": ["n01818515", "macaw"],
                   ...
                   "279": ["n02120079", "Arctic_fox"], "280": ["n02120505", "grey_fox"], "281": ["n02123045", "tabby"],
                   "282": ["n02123159", "tiger_cat"], "283": ["n02123394", "Persian_cat"],
                   ...
                   "393": ["n02607072", "anemone_fish"], "394": ["n02640242", "sturgeon"], "395": ["n02641379", "gar"],
                   "396": ["n02643566", "lionfish"], "397": ["n02655020", "puffer"], "398": ["n02666196", "abacus"],
                   ...
                   "428": ["n02797295", "barrow"], "429": ["n02799071", "baseball"], "430": ["n02802426", "basketball"],
                   ...
                   "484": ["n02981792", "catamaran"], "485": ["n02988304", "CD_player"], "486": ["n02992211", "cello"],
                   ...
                   "535": ["n03208938", "disk_brake"], "536": ["n03216828", "dock"], "537": ["n03218198", "dogsled"],
                   "538": ["n03220513", "dome"], "539": ["n03223299", "doormat"],
                   ...
                   "655": ["n03770439", "miniskirt"], "656": ["n03770679", "minivan"], "657": ["n03773504", "missile"],
                   ...
                   "678": ["n03814639", "neck_brace"], "679": ["n03814906", "necklace"], "680": ["n03825788", "nipple"],
                   ...
                   "782": ["n04152593", "screen"], "783": ["n04153751", "screw"], "784": ["n04154565", "screwdriver"],
                   ...
                   "817": ["n04285008", "sports_car"], "818": ["n04286575", "spotlight"], "819": ["n04296562", "stage"],
                   "820": ["n04310018", "steam_locomotive"], "821": ["n04311004", "steel_arch_bridge"],
                   "822": ["n04311174", "steel_drum"], "823": ["n04317175", "stethoscope"],
                   "824": ["n04325704", "stole"],
                   "825": ["n04326547", "stone_wall"], "826": ["n04328186", "stopwatch"], "827": ["n04330267", "stove"],
                   "828": ["n04332243", "strainer"], "829": ["n04335435", "streetcar"],
                   "830": ["n04336792", "stretcher"],
                   "831": ["n04344873", "studio_couch"], "832": ["n04346328", "stupa"],
                   ...
                   "857": ["n04429376", "throne"], "858": ["n04435653", "tile_roof"], "859": ["n04442312", "toaster"],
                   "860": ["n04443257", "tobacco_shop"], "861": ["n04447861", "toilet_seat"],
                   "862": ["n04456115", "torch"],
                   "863": ["n04458633", "totem_pole"], "864": ["n04461696", "tow_truck"],
                   "865": ["n04462240", "toyshop"],
                   "866": ["n04465501", "tractor"], "867": ["n04467665", "trailer_truck"], "868": ["n04476259", "tray"],
                   ...
                   "901": ["n04579145", "whiskey_jug"], "902": ["n04579432", "whistle"], "903": ["n04584207", "wig"],
                   ...
                   "963": ["n07873807", "pizza"], "964": ["n07875152", "potpie"], "965": ["n07880968", "burrito"],
                   ...
                   "972": ["n09246464", "cliff"], "973": ["n09256479", "coral_reef"], "974": ["n09288635", "geyser"],
                   "975": ["n09332890", "lakeside"], "976": ["n09399592", "promontory"],
                   "977": ["n09421951", "sandbar"],
                   "978": ["n09428293", "seashore"], "979": ["n09468604", "valley"], "980": ["n09472597", "volcano"],
...
class BaseDataset(Dataset, ABC):
    def __init__(self, train: bool = True, transform: torch.nn.Module = None, classes_subset: List[int] = None):
        super().__init__()
        self.transform = transform
        self.train = train
        self.classes_subset = classes_subset
        self.root_dir = self.get_root_dir()
        self.im_list, self.targets = self.get_images_and_target()
    ...
    @abstractmethod
    def get_images_and_target(self) -> Tuple[List[str], Tensor]:
        pass


class ImagenetDataset(BaseDataset):
    def __init__(self, train: bool = True, transform: torch.nn.Module = None, classes_subset: List[int] = None):
        super().__init__(train=train, transform=transform, classes_subset=classes_subset)

    def get_root_dir(self) -> str:
        root_dir = '/cs/labs/Academic/dataset/ILSVRC2012/train'
        root_dir += '/image' if self.train else '/val'
        return root_dir

    def get_images_and_target(self) -> Tuple[List[str], torch.Tensor]:
        self.classes = [v[CLASS_NAME_IND] for k, v in IMAGENET_LABELS.items()]
        self.classes = self.classes if self.classes_subset is None else [self.classes[i] for i in self.classes_subset]
        self.classes = IMAGENET_LABELS if self.classes_subset is None else {str(i): IMAGENET_LABELS[str(i)] for i in
                                                                            self.classes_subset}
        im_list = []
        targets = []
        for k, v in self.classes.items():
            class_folder_name = v[CLASS_NAME_IND]
            files = os.listdir(os.path.join(self.root_dir, class_folder_name))
            temp = sorted([os.path.join(class_folder_name, f) for f in files if f.endswith('JPEG')])
            im_list += temp
            targets += [int(k)] * len(temp)
        targets = torch.tensor(targets)
        return im_list, targets


class Places365Dataset(BaseDataset):
    def __init__(self, train=True, transform=None, classes_subset: List[int] = None):
        super().__init__(train=train, transform=transform, classes_subset=classes_subset)

    def get_root_dir(self) -> str:
        root_dir = '/cs/labs/Academic/dataset/Places2/'
        root_dir += '/data_large' if self.train else '/val_large'
        return root_dir
    ...
    def get_images_and_target(self) -> Tuple[List[str], torch.Tensor]:
        if self.train:
            all_images = []
            all_targets = []
            labels = self._get_labels_dirs()
            for dir, label in tqdm(labels.items()):
                if self.classes_subset is not None:
                    if label not in self.classes_subset:
                        continue
                path = self.root_dir + dir
                images = os.listdir(path)
                all_images += [path + "/" + img for img in images]
                all_targets += [label] * len(images)
        else:
            all_targets = self._get_val_targets()
            all_images = [f"Places365_val_000{str(i + 1).zfill(5)}.jpg" for i in range(len(all_targets))]
        return all_images, torch.tensor(all_targets)

    @staticmethod
    def _get_labels_dirs() -> Dict[str, int]:
        labels_file = "data_loaders/categories_places365.txt"
        with open(labels_file, "r") as f:
            lines = [l.strip().split(" ") for l in f.readlines()]
            labels = {l[0]: int(l[1]) for l in lines}
        return labels

    @staticmethod
    def _get_val_targets() -> List[int]:
        labels_file = "/cs/labs/yweiss/ofirshifman/clip/data_loaders/places365_val.txt"
        with open(labels_file, "r") as f:
            targets = [int(l.strip().split(" ")[1]) for l in f.readlines()]
        return targets


### Augmentations ###
class SingleImageCropsDataset(Dataset):
    def __init__(self, base_model: BaseDataset, image_idx: int = 0, crop_size=DEFAULT_CROP_SIZE,
                 base_image_size=DEFAULT_BASE_SIZE):
        super().__init__()
        self.base_model = base_model
        self.image_idx = image_idx
        assert base_image_size - crop_size > 0, "crop must be bigger than image base size"
        self.num_crops_in_row = base_image_size - crop_size
        self.num_crops = self.num_crops_in_row ** 2
        self.top = 0
        self.left = 0
    ...
    def __getitem__(self, idx):
        image, targets = self.base_model.__getitem__(self.image_idx)
        image = F.crop(image, self.top, self.left, self.crop_size, self.crop_size)
        self.left += 1
        if self.left % self.num_crops_in_row == 0:
            self.top += 1
            self.left = 0
        return image, self.base_model.targets[self.image_idx]


class SingleImageZoomDataset(Dataset):
    def __init__(self, base_model: BaseDataset, image_idx: int = 0, base_image_size:int = DEFAULT_BASE_SIZE,
                 input_size : int = DEFAULT_CROP_SIZE):
        super().__init__()
        self.base_model = base_model
        self.image_idx = image_idx
        self.h_cutout_size = 0
        self.w_cutout_size = 0
        self.base_size = base_image_size
        self.num_wh_vals = 32
        self.num_crops = self.num_wh_vals ** 2
    ...
    def __getitem__(self, idx):
        image, targets = self.base_model.__getitem__(self.image_idx)
        height = self.base_size - (self.h_cutout_size * 2)
        width = self.base_size - (self.w_cutout_size * 2)
        # print(f"h count: {self.h_cutout_size}", f"w count: {self.w_cutout_size}", f"height: {height}",f"width: {width}")
        image = F.crop(image, self.h_cutout_size, self.w_cutout_size, height=height,width=width)
        image = self.resize(image)
        self.w_cutout_size += 1
        if self.w_cutout_size % self.num_wh_vals == 0:
            self.h_cutout_size += 1
            self.w_cutout_size = 0
        return image, self.base_model.targets[self.image_idx]